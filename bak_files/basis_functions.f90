...
!  integer :: fact2 
  ...
  type gaussian
      ...
      real(8) :: origin(3)
      integer :: shell(3), nb, nbas, n_atm !nb => number of bare gaussians
...
contains !A problem, I had junk in kinetic function, looks to happen for coulomb too
  ...
  subroutine S_int(this, g1, g2, i, j)
     ...
     class(gaussian), intent(inout) :: g1, g2
       ...
       integer                      :: ia, ib !, i, j  !without i and j as arguments, it does overlap within two contracted gaussians
       real(8)                      :: overlap, s_val
       s_val = 0.0d0       
       do ia = 1, g1%nb
          do ib = 1, g2%nb
             !norm is a choice to normalize gaussians, in case of not add up to 1
             !s_val = s_val + g1%norm(ia) * g2%norm(ib) * g1%coef(ia,i) * g2%coef(ib,j) * & 
             s_val = s_val + g1%coef(ia,i) * g2%coef(ib,j) * &
     ...
     class(gaussian), intent(inout) :: g1, g2
       ...
       do ia = 1, g1%nb
          do ib = 1, g2%nb
             ...
             kin_val = kin_val + g1%coef(ia,i) * g2%coef(ib,j) * &
     ...
     class(gaussian), intent(inout) :: g1, g2
       ...
       do ia = 1, g1%nb 
          do ib = 1, g2%nb 
             ...
             V_col = V_col + g1%coef(ia,i) * g2%coef(ib,j) * &
